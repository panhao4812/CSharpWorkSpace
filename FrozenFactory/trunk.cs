...
namespace RhinoDebug
{
    class trunk : GH_ScriptInstance
    {
        ...
        #endregion
        private void RunScript(List<Line> x, List<Point3d> y, List<Mesh> z, ref object A)
        {
            try
            {
                ...
                for (int j = 0; j < vs.Count; j++)
                {
                    ...
                    {
                        double min = double.MaxValue; Point3d newpos = new Point3d();
                        for (int k = 0; k < z.Count; k++)
                        {
                            Mesh mesh = z[k];
                            double t = mesh.ClosestPoint(vs[j].pos).DistanceTo(vs[j].pos);
                            if (t < min) { min = t; newpos = y[k]; }
                        }
                        vs[j].pos = newpos;
                    }
                }
                ...
                {
                    output.Add(new Line(vs[id[i].I].pos, vs[id[i].J].pos));
